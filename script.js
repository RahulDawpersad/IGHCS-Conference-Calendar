document.addEventListener('DOMContentLoaded', function () {
    ...
    function completeBooking(paymentMethod) {
        ...
        // Prepare minimal booking details for Payfast
        ...
        // Compress the data to fit within PayFast's limits
        const compressedData = compressBookingData(bookingDetails);

        // Store full details in sessionStorage for after payment
        sessionStorage.setItem('currentBooking', JSON.stringify({
            ...bookingDetails,
            // Include full delegate details if company booking
            delegates: bookingType === 'company' ? getDelegateDetails() : null
        }));

        // Redirect to payment page with Payfast integration
        redirectToPayfast(compressedData, paymentMethod);

        // Add validation for the amount
        if (isNaN(bookingDetails.total) || bookingDetails.total <= 0) {
            alert('Invalid payment amount');
            return;
        }

        // Validate compressed data before proceeding
        // if (!compressedData || !compressedData.a) {
        //     alert('Error preparing payment details');
        //     return;
        // }
    }
    ...
    function redirectToPayfast(compressedData, paymentMethod) {
        try {
            // First, parse the compressedData if it's a string
            const data = typeof compressedData === 'string' ? JSON.parse(compressedData) : compressedData;

            // Validate the amount
            if (typeof data.a === 'undefined' || isNaN(data.a)) {
                throw new Error('Invalid amount in payment data');
            }

            // Your Payfast merchant credentials
            const merchant_id = '10033010';
            const merchant_key = '6dwmr589lfzbq';
            ...
            const paymentData = {
                ...
                cancel_url: window.location.origin + '/booking-cancel.html',
                notify_url: window.location.origin + '/payfast-notify', // THIS MUST BE CORRECT
                amount: parseFloat(data.a).toFixed(2),
                item_name: `${data.e} (${data.q} tickets)`,
                custom_str1: JSON.stringify(data),
                m_payment_id: data.r
            };

            // Add customer details if available
            if (data.t === 'i') {
                paymentData.name_first = (data.f || '').substring(0, 100);
                paymentData.name_last = (data.l || '').substring(0, 100);
                paymentData.email_address = data.m || '';
                paymentData.cell_number = data.p || '';
            } else {
                paymentData.name_first = (data.n || '').substring(0, 100);
                paymentData.email_address = data.m || '';
                paymentData.cell_number = data.p || '';
            }
    ...
    // You'll need to implement this function to generate the signature
    function generatePayfastSignature(data, merchantKey) {
        ...
        // Add your passphrase
        if (merchantKey) {
            pfOutput += `&passphrase=${encodeURIComponent(merchantKey.trim()).replace(/%20/g, '+')}`;
        }
            ...
            if (bookingDetails) {
                ...
                if (successContainer) {
                    successContainer.innerHTML = `
                        ...
                        <p>Booking Reference: ${bookingDetails.bookingRef}</p>
                        <p>Event: ${bookingDetails.event.title}</p>
                        <p>Amount: R ${bookingDetails.total.toLocaleString('en-ZA')}</p>
                    `;
                }

                // Send notifications
                sendPostPaymentNotifications(bookingDetails);
            }
        } catch (error) {
